  ...
  template: `
    ...
    <div class="salas-layout">
      <aside class="salas-sidebar" aria-label="Filtros de busqueda">
        ...
        <button class="reset-btn" (click)="resetFiltros()">
          Limpiar filtros
        </button>
      </aside>
      ...
      <main class="salas-main">
        <div class="salas-header">
          ...
          <div class="salas-count">
            {{ salasFiltradas().length }} disponible(s)
          </div>
        </div>
        ...
        <div class="salas-grid">
          ...
          >
            <div
              ...
              [ngStyle]="
                sala.imagenUrl
                  ? { 'background-image': 'url(' + sala.imagenUrl + ')' }
                  : {}
              "
            >
              <span *ngIf="!sala.imagenUrl" class="sala-img-label">{{
                sala.name
              }}</span>
              <button
                class="fav-btn"
            ...
            <div class="sala-body">
              <div class="sala-badge-row">
                ...
                <span class="sala-cap"
                  >{{ sala.capacidad }} músicos · {{ sala.m2 }}m²</span
                >
              </div>
...
export class SalasComponent {
  ...
  esFavorito(salaId: string): boolean {
    ...
    return usuarioId
      ? this.favoritosService.esFavorito(String(usuarioId), salaId)
      : false;
  }
  ...
  salasFiltradas = () =>
    this.salas.filter((s) => {
      ...
      const okCap =
        this.filtroCapacidad() === 0 || s.capacidad >= this.filtroCapacidad();
