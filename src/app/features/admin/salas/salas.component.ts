  ...
  template: `
    <div class="panel-header">
      <div>
        ...
        <p class="panel-subtitle">
          Administra el catálogo completo de salas: crea, edita o elimina,
    ...
    <div class="panel-card sala-edit-card" *ngIf="mostrarFormularioNuevo()">
      ...
      <div class="sala-edit-footer">
        <span class="guardado-msg" *ngIf="guardadoId() === 'nueva'"
          >✓ Sala creada</span
        >
    ...
    <div class="salas-admin-grid">
      <div class="panel-card sala-edit-card" *ngFor="let s of salas()">
        ...
        <div class="sala-edit-footer">
          <span class="guardado-msg" *ngIf="guardadoId() === s.id"
            >✓ Guardado</span
          >
          <button
            class="submit-btn"
      ...
      <div class="panel-empty" *ngIf="salas().length === 0">
        No hay salas registradas todavía. Usa «+ Nueva sala» para crear la
        primera.
      </div>
    </div>
      ...
      <div class="form-grid">
        ...
        <div class="np-field">
          ...
          <input
            ...
            type="number"
            min="1"
            [(ngModel)]="edicion.capacidad"
            [name]="'capacidad-' + sufijo"
          />
        </div>
        <div class="np-field">
        ...
        <div class="np-field">
          ...
          <input
            [id]="'badgeLabel-' + sufijo"
            type="text"
            [(ngModel)]="edicion.badgeLabel"
            [name]="'badgeLabel-' + sufijo"
          />
        </div>
        <div class="np-field checkbox-field">
          <label>
            <input
              type="checkbox"
              [(ngModel)]="edicion.featured"
              [name]="'featured-' + sufijo"
            />
            Destacada
          </label>
        </div>
        <div class="np-field full">
          ...
          <textarea
            ...
            [name]="'descripcion-' + sufijo"
          ></textarea>
        </div>
        <div class="np-field full">
          <label [for]="'equipo-' + sufijo"
            >Equipo incluido (un elemento por línea)</label
          >
          <textarea
            [id]="'equipo-' + sufijo"
            rows="4"
            [(ngModel)]="edicion.equipoTexto"
            [name]="'equipo-' + sufijo"
          ></textarea>
        </div>
      </div>
...
export class AdminSalasComponent {
  ...
  constructor() {
    ...
    this.salasService.cargarSalas().catch((err) => {
      this.errorGlobal.set(
        mensajeDeError(err, 'No se pudieron cargar las salas.'),
      );
    });
  }
  ...
  crearHandlerArchivo(salaId: string): (evento: Event) => void {
    if (!this.handlersArchivo[salaId]) {
      this.handlersArchivo[salaId] = (evento: Event) => {
        ...
        if (archivo) {
          this.previewsPorId.update((p) => ({
            ...p,
            [salaId]: URL.createObjectURL(archivo),
          }));
        }
      };
    }
  ...
  async guardar(salaId: string): Promise<void> {
    ...
    try {
      ...
      this.errorGlobal.set(
        mensajeDeError(err, 'No se pudieron guardar los cambios de la sala.'),
      );
    } finally {
  ...
  async eliminar(salaId: string, nombre: string): Promise<void> {
    const confirmado =
      ...
      window.confirm(
        `¿Eliminar la sala "${nombre}"? Esta acción no se puede deshacer.`,
      );
    ...
    try {
      ...
      if (forzar) {
        try {
          ...
          this.errorGlobal.set(
            mensajeDeError(err2, 'No se pudo eliminar la sala.'),
          );
        }
      } else {
